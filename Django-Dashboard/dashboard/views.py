...
MONGODB_DB_NAME = os.getenv("MONGODB_DB_NAME", "big_data_project")
...
def plot_word_frequencies_per_class(data):
    ...
    static_folder = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "BigDataProject", "static", "imgs"
    )
...
def dashboard(request):
    ...
    sentiment_counts = {
        label: 0 for label in ["Negative", "Positive", "Neutral", "Irrelevant"]
    }
    for entry in data:
    ...
    sentiment_rates = {
        label: round((count / len_data) * 100, 2)
